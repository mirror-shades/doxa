...
pub const PeepholeOptimizer = struct {
    ...
    fn tryRedundantElimination(self: *PeepholeOptimizer, instructions: []HIRInstruction, i: usize, optimized: *std.ArrayList(HIRInstruction)) !?usize {
        ...
        // Pattern 1: Detect chain of Dups followed by Peek + Pop → remove Dups, keep Peek + Pop
        if (std.meta.activeTag(instructions[i]) == .Dup) {
            ...
            {
                // Replace Dups with nothing, but preserve Peek + Pop to keep stack balanced
                try optimized.append(instructions[j]); // Peek
                try optimized.append(instructions[j + 1]); // Pop
                self.redundant_eliminations += @intCast(dup_count);
                return dup_count + 2; // Consumed: Dups + Peek + Pop
            }
        }

        // Pattern 2: Single Dup + Peek + Pop → Peek + Pop
        ...
        {
            try optimized.append(instructions[i + 1]); // Peek
            try optimized.append(instructions[i + 2]); // Pop
            self.redundant_eliminations += 1;
            return 3;
        }

        // Pattern 2b: Dup + ArrayGet + Peek + Pop → ArrayGet + Peek + Pop
            ...
            std.meta.activeTag(instructions[i + 3]) == .Pop)
        {
            ...
            try optimized.append(instructions[i + 3]); // Pop
            self.redundant_eliminations += 1;
            return 4;
        }

        // Pattern 3: LoadVar + Dup + Peek + Pop → LoadVar + Peek + Pop
        if (i + 3 < instructions.len and
