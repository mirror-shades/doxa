    ...
    fn generateStatement(self: *HIRGenerator, stmt: ast.Stmt) (std.mem.Allocator.Error || reporting.ErrorList)!void {
        switch (stmt.data) {
            ...
            .VarDecl => |decl| {
                ...
                // Store variable (single instruction). Do not duplicate the value;
                // declarations should not leave residual values on the stack.
    ...
    fn generateExpression(self: *HIRGenerator, expr: *ast.Expr, preserve_result: bool) !void {
        switch (expr.data) {
            ...
            .If => |if_expr| {
                ...
                // Propagate preserve_result into branches so statement-context expressions
                // don't retain values on the stack
                try self.generateExpression(if_expr.then_branch.?, preserve_result);
                ...
                if (if_expr.else_branch) |else_branch| {
                    try self.generateExpression(else_branch, preserve_result);
                } else {
                ...
                // If the overall If-expression result is not needed, drop it now
                if (!preserve_result) {
                    try self.instructions.append(.Pop);
                }
            },
            ...
            .While => |while_expr| {
                ...
                // In statement context, drop the synthetic result to keep the stack clean
                if (!preserve_result) {
                    try self.instructions.append(.Pop);
                }
            },
