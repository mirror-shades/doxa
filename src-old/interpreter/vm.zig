    ...
    fn executeInstruction(self: *HIRVM, instruction: HIRInstruction) ErrorList!void {
        switch (instruction) {
            ...
            .StoreVar => |v| {
                ...
                // Enforce type safety: prevent accidental overwrites with wrong types
                if (v.expected_type != .Auto) {
                    const actual_type: HIRType = switch (coerced_value) {
                        .int => .Int,
                        .byte => .Byte,
                        .float => .Float,
                        .string => .String,
                    ...
                    // For arrays, only accept array values
                    if (v.expected_type == .Array and actual_type != .Array) {
                        return self.reporter.reportError("Type mismatch storing {s}: expected array, got {s}", .{ v.var_name, @tagName(coerced_value) });
                    }

                    // For non-array primitive types, enforce exact match
                    if (v.expected_type != .Array and v.expected_type != actual_type) {
                        return self.reporter.reportError("Type mismatch storing {s}: expected {s}, got {s}", .{ v.var_name, @tagName(v.expected_type), @tagName(coerced_value) });
                    }
                }
                ...
                if (std.mem.eql(u8, v.var_name, "resint") or std.mem.eql(u8, v.var_name, "resfloat") or std.mem.eql(u8, v.var_name, "tape")) {
                    self.reporter.debug(
